...
export class DingtalkAdapter {
     ...
     */
    async downloadByCode(downloadCode, robotCode) {
        try {
            ...
            console.error('[dingtalk] Failed to download attachment:', e.message);
            return null;
        }
    }
    /** Download an image via downloadCode (two-step API) or a direct picURL. */
    async downloadImage(downloadCode, picURL, robotCode) {
        if (downloadCode) {
        ...
        if (picURL) {
            try {
                ...
                console.error('[dingtalk] Failed to download image URL:', e.message);
                return null;
            }
        }
        return null;
    ...
    async start(onMessage) {
        ...
        this.dwClient.registerCallbackListener(TOPIC_ROBOT, async (res) => {
            ...
            const robotCode = this.config.robotCode || data.robotCode || this.config.clientId;
            let text = '';
            const images = [];
            const files = [];
