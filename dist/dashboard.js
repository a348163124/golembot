...
export function recordMessage(metrics, msg) {
    metrics.totalMessages++;
    if (msg.costUsd)
        metrics.totalCostUsd += msg.costUsd;
    if (msg.durationMs)
        metrics.totalDurationMs += msg.durationMs;
    metrics.messagesBySource[msg.source] = (metrics.messagesBySource[msg.source] ?? 0) + 1;
    metrics.recentMessages.push(msg);
    if (metrics.recentMessages.length > MAX_RECENT)
...
export async function buildDashboardData(ctx) {
...
// ── Escalation data ─────────────────────────────────────────────────────────
async function loadEscalations(dir) {
    if (!dir)
        return [];
    try {
...
async function loadActiveSessions(dir) {
    if (!dir)
        return [];
    try {
        ...
        if (!Array.isArray(data))
            return [];
        return data
            .filter((s) => s.key && s.lastActivity)
            .slice(-20)
            .map((s) => ({ key: String(s.key), lastActivity: String(s.lastActivity) }));
    }
    catch {
        return [];
    }
...
async function loadMemoryOverview(dir) {
    if (!dir)
        return undefined;
    let notesPreview;
    try {
    ...
    if (!notesPreview && !groupFiles && !recentSummaries)
        return undefined;
    return { notesPreview, groupFiles, recentSummaries };
}
async function loadFleetPeers(ctx) {
    if (!ctx.getFleetPeers)
        return undefined;
    try {
...
function configBadge(val, defaultVal = false) {
...
function configVal(val, defaultVal, suffix = '') {
 ...
 */
function editable(html, path, currentVal, type = 'text', options) {
...
function renderConfigPanel(data) {
    ...
    if (cfg.systemPrompt) {
        ...
        advancedParts.push(`<div class="config-row"><span class="config-key">System Prompt</span><pre class="config-prompt">${esc(preview)}</pre></div>`);
    }
    if (mcp && Object.keys(mcp).length > 0) {
        const mcpRows = Object.entries(mcp)
            .map(([name, srv]) => `<div class="config-row"><span class="config-key" style="min-width:auto">${esc(name)}</span><code>${esc(srv.command)}${srv.args ? ` ${srv.args.map(esc).join(' ')}` : ''}</code></div>`)
            .join('');
        advancedParts.push(`<div class="config-sub-label">MCP Servers</div>${mcpRows}`);
    }
    if (inbox) {
        advancedParts.push(`<div class="config-row"><span class="config-key">Inbox</span>${configBadge(inbox.enabled)} <span class="dim" style="margin-left:8px">retention: ${inbox.retentionDays ?? 7} days</span></div>`);
    }
    if (hf) {
        advancedParts.push(`<div class="config-row"><span class="config-key">History Fetch</span>${configBadge(hf.enabled)} <span class="dim" style="margin-left:8px">poll: ${hf.pollIntervalMinutes ?? 15} min</span></div>`);
    }
    if (esc_) {
        const targetDesc = esc_.target
            ? `${esc_.target.channel}:${(esc_.target.chatId ?? 'all').slice(0, 16)}`
            : 'no target';
        advancedParts.push(`<div class="config-row"><span class="config-key">Escalation</span>${configBadge(esc_.enabled)} <span class="dim" style="margin-left:8px">${esc(targetDesc)}</span></div>`);
    }
    const advancedSection = advancedParts.length > 0
...
// ── Fleet Peers Panel ───────────────────────────────────────────────────────
function renderFleetPeers(data) {
    if (!data.fleetPeers || data.fleetPeers.length === 0)
        return '';
...
function renderChannelRow(ch, type) {
...
function renderAccessCards(data) {
    ...
    const knownRows = KNOWN_CHANNELS.map((type) => renderChannelRow(data.channels.find((c) => c.type === type), type)).join('\n');
    ...
    const rows = data.tasks
        ...
        const histHtml = hist.length > 0
            ? `<div class="task-hist">${hist
                ...
                const hStatus = h.status === 'success'
...
function renderClientScript(data) {
...
function renderPersonaCard(data) {
    if (!data.persona || (!data.persona.displayName && !data.persona.role))
        return '';
    const rows = [];
    if (data.persona.displayName)
        rows.push(`<div class="persona-row"><span class="persona-label">Display Name</span><span>${esc(data.persona.displayName)}</span></div>`);
    if (data.persona.role)
        rows.push(`<div class="persona-row"><span class="persona-label">Role</span><span>${esc(data.persona.role)}</span></div>`);
    if (data.persona.tone)
        rows.push(`<div class="persona-row"><span class="persona-label">Tone</span><span>${esc(data.persona.tone)}</span></div>`);
    if (data.persona.boundaries?.length) {
        rows.push(`<div class="persona-row"><span class="persona-label">Boundaries</span><span>${data.persona.boundaries.map((b) => esc(b)).join(', ')}</span></div>`);
    }
    return `
<div class="card" style="margin-bottom:24px">
...
function renderSkillInventory(data) {
    ...
    if (!hasTypes) {
        const rows = data.skills
            .map((s) => `<div class="skill-row"><span class="skill-name">${esc(s.name)}</span><span class="skill-desc">${esc(s.description)}</span></div>`)
            .join('\n');
        return `
<div class="card" style="margin-bottom:24px">
...
function renderActiveSessionsPanel(data) {
    ...
    const rows = data.activeSessions
        ...
        .reverse()
        .slice(0, 10)
        .map((s) => `<div class="session-row"><span class="session-key">${esc(s.key)}</span><span class="session-time">${esc(s.lastActivity)}</span></div>`)
        .join('\n');
    return `
<div class="card" style="margin-bottom:24px">
...
function renderMemoryOverview(data) {
    if (!data.memoryOverview)
        return '';
    const sections = [];
    if (data.memoryOverview.notesPreview) {
        sections.push(`<div class="memory-section"><h3>Notes</h3><pre class="memory-preview">${esc(data.memoryOverview.notesPreview)}${data.memoryOverview.notesPreview.length >= 300 ? '…' : ''}</pre></div>`);
    }
    if (data.memoryOverview.groupFiles?.length) {
        sections.push(`<div class="memory-section"><h3>Memory Groups</h3><div class="memory-list">${data.memoryOverview.groupFiles.map((f) => `<span class="memory-file">${esc(f)}</span>`).join('')}</div></div>`);
    }
    if (data.memoryOverview.recentSummaries?.length) {
        sections.push(`<div class="memory-section"><h3>Recent Summaries</h3><div class="memory-list">${data.memoryOverview.recentSummaries.map((f) => `<span class="memory-file">${esc(f)}</span>`).join('')}</div></div>`);
    }
...
function renderEscalationPanel(data) {
    ...
    const rows = data.escalations
        ...
        .reverse()
        .map((e) => {
        const statusBadge = e.status === 'resolved' ? '<span class="task-ok">resolved</span>' : '<span class="task-err">open</span>';
