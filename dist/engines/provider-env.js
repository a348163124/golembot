...
export function codexProviderEnv(provider) {
    const env = {};
    if (provider.apiKey) {
...
export function cursorProviderEnv(provider) {
    const env = {};
    if (provider.apiKey)
        env.CURSOR_API_KEY = provider.apiKey;
    if (provider.baseUrl)
...
export function openCodeProviderEnv(provider, model) {
    const env = {};
    if (provider.apiKey) {
        const providerPrefix = model?.split('/')[0] || 'openrouter';
        const envVar = OPENCODE_PROVIDER_ENV[providerPrefix] || `${providerPrefix.toUpperCase().replace(/-/g, '_')}_API_KEY`;
