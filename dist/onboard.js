...
function detectEngineAuth(engine) {
    if (engine === 'codex') {
        if (process.env.CODEX_API_KEY)
            return { ok: true, detail: 'CODEX_API_KEY' };
        if (process.env.OPENAI_API_KEY)
            return { ok: true, detail: 'OPENAI_API_KEY' };
        const oauthFile = join(homedir(), '.codex', 'auth.json');
        if (existsSync(oauthFile))
            return { ok: true, detail: 'ChatGPT OAuth (~/.codex/auth.json)' };
        return { ok: false, detail: '' };
    }
    if (engine === 'claude-code') {
        if (process.env.ANTHROPIC_API_KEY)
            return { ok: true, detail: 'ANTHROPIC_API_KEY' };
        return { ok: false, detail: '' };
    }
    if (engine === 'cursor') {
        if (process.env.CURSOR_API_KEY)
            return { ok: true, detail: 'CURSOR_API_KEY' };
        return { ok: false, detail: '' };
    }
    if (engine === 'opencode') {
...
export function generateEnvExample(engine, channels) {
    const lines = ['# GolemBot environment variables', ''];
...
export async function installTemplate(dir, templateName, templatesBase) {
...
export async function runOnboard(opts = {}) {
    ...
    const depsToInstall = channels.flatMap((ch) => CHANNEL_DEPS[ch] || []);
