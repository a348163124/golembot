...
const pkgVersion = JSON.parse(readFileSync(join(__dirname_cli, '..', 'package.json'), 'utf-8')).version;
...
const BOX_WIDTH = 52;
function boxLine(content, rawLen) {
...
function centerArt(line, rawLen) {
...
program
    ...
    // ── Welcome banner ──
    {
        ...
        try {
            ...
            if (skills.length > 0) {
                ...
                if (skillStr.length > maxValLen)
                    skillStr = `${skillStr.slice(0, maxValLen - 1)}\u2026`;
    ...
    const doPrompt = () => {
        ...
        rl.question('> ', async (input) => {
            ...
            if (trimmed === '"""') {
                const lines = [];
                const collectLine = () => new Promise((r) => rl.question('... ', r));
                while (true) {
...
program
    ...
    try {
        ...
        console.error(`❌ Failed to read assistant status: ${e.message}`);
...
skill
    ...
    for (const s of results) {
        ...
        if (meta)
            console.log(`  ${' '.repeat(30)} ${DIM}${meta}${RESET}`);
    }
